...
const repository = element('a', 'Public repository ↗', 'repository-link'); repository.href = 'https://github.com/W3Booster/app-resource-monitor'; repository.target = '_blank'; repository.rel = 'noopener noreferrer';
...
for (const [text, href] of [['Build your own', 'https://website.w3booster.com/developer/first-app/'], ['View source', 'https://github.com/W3Booster/app-resource-monitor'], ['SDK reference', 'https://website.w3booster.com/developer/api/']]) {
  const link = element('a', text); link.href = href; footer.append(link);
}
const sourceLink = element('a', 'Read this example’s code ↗', 'source-link'); sourceLink.href = 'https://github.com/W3Booster/app-resource-monitor/blob/main/src/render.ts'; sourceLink.target = '_blank'; sourceLink.rel = 'noopener noreferrer'; controls.append(sourceLink);
