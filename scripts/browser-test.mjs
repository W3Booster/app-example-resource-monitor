...
try {
  ...
  if (config.slug === 'resource-monitor') {
    ...
    let recorder;
    let sequence = 0;
    const sendState = () => broker.send(JSON.stringify({ version: '4.0', type: 'state.snapshot', sequence: ++sequence, data: state }));
    await live.route('**/stream/v1/stream-tickets', route => {
      ...
      return route.fulfill({ json: { websocketUrl: 'wss://stream.example.test/apps', protocolVersion: '4.0', applicationRevision: request.applicationRevision } });
    });
